...
class GrayToClearPictureRenderer implements Renderer<Picture> {
	/**
	...
	render(info : Picture, domElem : any, rendererTheme : string, endCallback : Function) {
		...
		pictureWrapper.addClass("GrayToClearPictureRenderer_wrapper");
		pictureWrapper.addClass(rendererTheme);

		var pictureTitle = $("<div>");
		pictureTitle.addClass("GrayToClearPictureRenderer_title");
		...
		pictureContainer.addClass("GrayToClearPictureRenderer_container");
		...
		pictureBackgroundContainer.addClass("GrayToClearPictureRenderer_picture_background");

		pictureContainer.append(pictureBackgroundContainer);

		var pictureHTML = $("<div>");
		pictureHTML.addClass("GrayToClearPictureRenderer_picture");

		pictureContainer.append(pictureHTML);

		if(picURL != null) {
			pictureBackgroundContainer.css("background-image", "url('" + picURL.getURL() + "')");
			pictureHTML.css("background-image", "url('" + picURL.getURL() + "')");
		}

		$(domElem).append(pictureWrapper);

		pictureHTML.css("filter", "grayscale(100%)");
	...
	animate(info : Picture, domElem : any, rendererTheme : string, endCallback : Function) {
		...
		var pictureBackElems = $(domElem).find(".GrayToClearPictureRenderer_picture_background");
		var pictureElems = $(domElem).find(".GrayToClearPictureRenderer_picture");

		pictureBackElems.first().transition({filter: "grayscale(100%) blur(10px)"}, 3500);
		pictureElems.first().transition({filter: "grayscale(0%)"}, 3500);
